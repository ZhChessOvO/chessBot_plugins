...
msg_everyday_max="你今天已经抽过签了哦，明天再来吧！"
--超过最大抽卡次数回复
...
random_lucky = lucky[math.random(#lucky)]
random_item = item[math.random(#item)]

--以上为自定义部分
msg_order={}
function jrrp_draw_main(msg)
    if(msg.fromMsg==draw_msg)then
    if(getUserToday(msg.fromQQ, "jrrp_today_draw_max", 0)<jrrp_everyday_max)then
      if(kj_kind==1)then
         kj_pic="[pic="
      else
         kj_pic="[CQ:image,url="
      end
      setUserToday(msg.fromQQ, "jrrp_today_draw_max", getUserToday(msg.fromQQ, "jrrp_today_draw_max", 0)+1)
      return "象棋为{nick}解签如下：\n"..random_lucky..random_item
    else
...
function jrrp_draw_state(msg)
   if(msg.fromQQ==master_qq)then
      return "管理员权限："..master_qq.."\n版本：v1.0".."\n每日最大抽签次数："..jrrp_everyday_max.."\n抽签指令："..draw_msg
   else
      return "你不是抽签管理员"
   end
end

function jrrp_draw_master(msg)
   if(msg.fromQQ==master_qq)then
      local user_qq=string.sub(msg.fromMsg,7,-1)
      setUserToday(user_qq, "jrrp_today_draw_max", 0)
      return "成功重置了[CQ:at,qq="..user_qq.."]("..user_qq..")今日的抽签行为"
   else
      return ""
   end
end
msg_order[draw_msg]="jrrp_draw_main"
msg_order["抽签重置"]="jrrp_draw_master"
msg_order[draw_state]="jrrp_draw_state"