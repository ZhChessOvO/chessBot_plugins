...
function searchColor(msg)
    local code = string.match(msg.fromMsg,"^[%s]*(.-)[%s]*$",7)
    if #code~=6 then
        return "参数非法，请认真输入色号哦"
    end
    local redcode = string.sub(code,1,2)
    local greencode = string.sub(code,3,4)
    local bluecode = string.sub(code,5,6)
    -- return redcode.." "..greencode.." "..bluecode
	Red = tonumber(redcode,16)
	Green = tonumber(greencode,16)
	Blue = tonumber(bluecode,16)
